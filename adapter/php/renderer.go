...
func (t *PHPTemplates) Exec() (string, map[string]string) {
	...
	// 整个模板写在一个反引号字符串里，除了 {v[0]} / {v[1]} / {v[2]} 三个随机
	// 字段名需要拼接外，其余 PHP 源码原样输出。
	code := "" +
		// 读取参数
		...
		// 构造 shell 参数：Linux 用 -c "..."，Windows 用 /c "..."
...
// FileList 返回一段 PHP 目录列表代码，路径从 base64 POST 参数读。
