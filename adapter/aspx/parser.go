...
func parseInfo(body []byte) core.Result {
	raw := string(body); parts := splitTab(raw)
	res := &core.InfoResult{BaseResult: core.NewBaseResult("info", body)}
	switch {
	case len(parts) >= 4: res.Workdir = fromB64(parts[0]); res.OS = fromB64(parts[2]); res.User = fromB64(parts[3])
	case len(parts) >= 3: res.Workdir = fromB64(parts[0]); res.OS = fromB64(parts[1]); res.User = fromB64(parts[2])
	case len(parts) >= 1: res.OS = fromB64(parts[0])
	}
	return res
}
...
func parseExec(body []byte) core.Result {
	...
	if idx := strings.LastIndex(s, prefix); idx >= 0 {
		...
		if code, err := atoi(codeStr); err == nil { out.ExitCode = code; out.Stdout = strings.TrimSpace(s[:idx]) }
	}
	return out
}
...
func b64DecodeBytes(src []byte) []byte {
	...
	if err != nil { out := make([]byte, len(src)); copy(out, src); return out }
	return dst[:n]
}

func splitTab(s string) []string {
	var out []string; cur := ""
	for _, r := range s { if r == '\t' { out = append(out, cur); cur = ""; continue }; cur += string(r) }
...
func splitLines(s string) []string {
	...
	for _, r := range s {
		...
		if r == '\r' { continue }; cur += string(r)
	}
	if cur != "" { out = append(out, cur) }; return out
}

func cutTab(s string) (string, string, bool) {
	for i, r := range s { if r == '\t' { return s[:i], s[i+1:], true } }; return "", "", false
}

func trimRight(s string, cuts string) string {
loop:
	for len(s) > 0 {
		c := s[len(s)-1:]; for _, r := range cuts { if string(r) == c { s = s[:len(s)-1]; continue loop } }; return s
	}
	return s
}
...
func atoi(s string) (int, error) {
	if s == "" { return 0, nil }; n := 0; neg := false; start := 0
	if s[0] == '-' { neg = true; start = 1 }
	if start >= len(s) { return 0, nil }
	for _, c := range s[start:] { if c < '0' || c > '9' { return 0, nil }; n = n*10 + int(c-'0') }
	if neg { n = -n }; return n, nil
}
